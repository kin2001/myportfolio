...
import { getMonitoringSnapshot, type MonitoringSnapshot } from "@/lib/monitoring";
import { createSupabaseServerClient } from "@/lib/supabase/server";

type MonitoringPromise = Promise<MonitoringSnapshot>;
      ...
      <div aria-hidden="true" className="mt-5 grid gap-5 md:grid-cols-2">
        {Array.from({ length: 2 }, (_, index) => (
          <div className="module min-h-56 p-6" key={index}>
            ...
            <div className="mt-4 h-4 w-4/5 bg-[var(--line)]" />
            <div className="mt-4 h-4 w-3/5 bg-[var(--line)]" />
          </div>
        ))}
      </div>
    </section>
...
async function TrafficSection({ monitoringPromise }: { monitoringPromise: MonitoringPromise }) {
...
function MonitoringRowsLoading() {
  return (
    <div>
...
async function MonitoringRows({
  ...
  monitoringPromise: MonitoringPromise;
  ...
  return (
    <>
      <div className="grid gap-2 border-t border-[var(--line)] p-5 md:grid-cols-[180px_130px_1fr]">
      ...
      <div className="grid gap-2 border-t border-[var(--line)] p-5 md:grid-cols-[180px_130px_1fr]">
        ...
        <span className={`mono-label ${monitoring.deployment.available ? "accent" : "muted"}`}>
          {monitoring.deployment.available ? deploymentData?.state : "UNAVAILABLE"}
        </span>
        <span className="text-sm ink-soft">
          {deploymentData ? (
      ...
      {monitoring.monitors.map((monitor) => (
        <div className="grid gap-2 border-t border-[var(--line)] p-5 md:grid-cols-[180px_130px_1fr]" key={monitor.label}>
...
export default async function AdminDashboard() {
  ...
  const databasePromise = supabase
    ? Promise.all([
        supabase.from("projects").select("lifecycle_state"),
        ...
        supabase
          .from("audit_events")
          .select("id,action,entity_type,entity_id,occurred_at", { count: "exact" })
          .order("occurred_at", { ascending: false })
          .limit(5),
      ])
    : Promise.resolve(null);
  const monitoringPromise = getMonitoringSnapshot(supabase ?? undefined);
  const database = await databasePromise;

  const databaseAvailable = database?.every((result) => !result.error) ?? false;
  const projectRows = databaseAvailable ? database![0].data ?? [] : [];
  const projectCount = (state: "draft" | "published" | "archived") =>
    databaseAvailable
      ? projectRows.filter((project) => project.lifecycle_state === state).length
      : null;
  const counts = [
    projectCount("published"),
    projectCount("draft"),
    projectCount("archived"),
    databaseAvailable ? database![1].count ?? 0 : null,
  ];
  const cvRows = databaseAvailable ? database![2].data ?? [] : [];
  const assetRows = databaseAvailable ? database![3].data ?? [] : [];
  const settings = databaseAvailable ? database![5].data : null;
  ...
  const deploymentCheck = (databaseAvailable ? database![4].data : null) as DeploymentCheck;
  const recentActivity = databaseAvailable ? database![6].data ?? [] : [];
  ...
  return (
    <div className="max-w-6xl">
      ...
      <Suspense fallback={<TrafficLoading />}>
        <TrafficSection monitoringPromise={monitoringPromise} />
      </Suspense>
      ...
      <section className="mt-12">
        ...
        <div className="module mt-5 divide-y divide-[var(--line)]">
          ...
          <Suspense fallback={<MonitoringRowsLoading />}>
            <MonitoringRows
              deploymentCheck={deploymentCheck}
              monitoringPromise={monitoringPromise}
            />
