...
export default async function HomePage() {
  ...
  return (
    <>
      ...
      />
    <div className={`${styles.home} mx-auto max-w-[1100px] space-y-10 px-4 py-8 sm:px-6 md:space-y-14 md:px-8 md:py-12`} data-homepage>
      <section data-reveal="hero" className={`${styles.hero} grid grid-cols-[minmax(0,1.2fr)_minmax(0,.8fr)] items-center gap-3 sm:gap-6 md:gap-x-10`} aria-labelledby="hero-title">
      ...
      <section className="space-y-8 md:space-y-10" id="projects">
        <div data-reveal="rule" className={`${styles.sectionHeading} flex items-center justify-between gap-3 border-b border-[var(--line)] pb-4`}><h2 className="mono-label tracking-[.2em]">01 — Projects</h2><Link href="/work" className="hero-link">View all {String(published.length).padStart(2, "0")}<span className="sr-only"> projects</span> <Icon name="arrow" /></Link></div>
        {published.length ? (
